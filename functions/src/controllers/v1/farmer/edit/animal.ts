...
export const addFarmerAnimal = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res.status(201).json({
      ...
      data: await AnimalInfoResponse(saved),
    });
  } catch (error) {
...
export const destroyFarmerAnimal = async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { animalId } = req.params;
    if (!animalId) return res.status(400).json({ message: "Animal id is required" });
