...
/**
 * Shared function to fetch recently updated farmer records data
 * @param {string} updatedFrom - Start date for the update range
 * @param {string} updatedTo - End date for the update range
 * @return {Promise<any>} Object containing summary and farmers data
 */
export const fetchRecentlyUpdatedFarmerData = async (updatedFrom: string, updatedTo: string) => {
  let fromDate = new Date(updatedFrom);
  let toDate = new Date(updatedTo);

  if (isNaN(fromDate.getTime()) || isNaN(toDate.getTime())) {
    throw new Error("Invalid date format. Use ISO 8601 format (e.g., 2024-01-01 or 2024-01-01T00:00:00Z)");
  }

  if (fromDate > toDate) {
    throw new Error("'updatedFrom' date must be before or equal to 'updatedTo' date");
  }
...
export const getRecentlyUpdatedFarmerRecords = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    // Use the shared function to fetch data
