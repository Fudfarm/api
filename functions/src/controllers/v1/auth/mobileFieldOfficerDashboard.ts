...
export const mobileFieldOfficerDashboard = async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const dailyAggThis = await User.aggregate([
      { $match: { role: "Farmer", createdBy: req.user?.id, createdAt: { $gte: startOfThisMonth, $lte: endOfThisMonth } } },
      { $project: { day: { $dayOfMonth: "$createdAt" } } },
      { $group: { _id: "$day", count: { $sum: 1 } } },
    ]).exec();

    const dailyAggLast = await User.aggregate([
      { $match: { role: "Farmer", createdBy: req.user?.id, createdAt: { $gte: startOfLastMonth, $lte: endOfLastMonth } } },
      { $project: { day: { $dayOfMonth: "$createdAt" } } },
      { $group: { _id: "$day", count: { $sum: 1 } } },
    ]).exec();
